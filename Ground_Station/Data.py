...
class Data:
    def __init__(self, serial_data_list):
        ...
        self.cam1 = self.get_idx(attribute_idx["cam1"])
        self.cam2 = self.get_idx(attribute_idx["cam2"])
        self.idx_attribute = {
            0: self.teamID,
            1: self.mission_time,
            2: self.packet_count,
    ...
    def get_idx(self, idx):
        serial_data = self.serial_data_list
        # Uncomment the below code if you pass a comma separated string to the Data class
        # data = serial_data.split(",")[idx]
        # data = data.strip(" ")
        # data = data.strip("'")
        data = serial_data[idx]
    ...
    def get_parsed_data(self):
        ...
        for idx in range(len(self.idx_attribute)):
            self.parsed_data[key_idx[idx]] = self.idx_attribute[idx]
    ...
    def convert_to_format(self, data, datatype):
        ...
        if datatype == "int":
            data = int(data)
        elif datatype == "float-1":
            data = round(float(data), 1)
        elif datatype == "float-2":
            data = round(float(data), 2)
        elif datatype == "float-4":
