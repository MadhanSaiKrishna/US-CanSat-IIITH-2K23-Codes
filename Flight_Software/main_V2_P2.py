...
############### ONLY IN P2 #####################
from reciever import receive_message
###############################################

#################### UNCOMMENT LATER ####################
from PC_Servo import parachute_servo
# from HS_deploy_servo import HS_deploy_servo # P1 - deploys HS; P2 - releases HS
# from HS_release_servo import HS_release_servo # P1 - deploys HS; P2 - releases HS
...
def send_telemetry(parsed_data):
    ...
    #################### UNCOMMENT LATER ####################
    # gl_cansat.send_data(gnd_station, parsed_data)
    #######################################################
    gl_packets_sent += 1
    print(data_to_send)
    return 0
...
##################### ONLY IN P2 ############################
def get_message_from_P1():
    """
    This function will get the message from the primary processor
    """
    global gl_message_fragmets_P1, gl_received_P1, gl_received_message_P1, gl_received_fragments_P1
    data = receive_message()
    gl_message_fragmets_P1.append(data)

    if "END_TELEMETRY" in data:
        gl_received_P1 = True
        gl_received_message_P1 = ",".join(gl_received_fragments_P1)
        gl_received_message_P1 = []
    
    datapt = gl_received_message_P1.split(',')[:-1]
...
def get_sensor_data():
    ...
    # Store sensor data
    data = {}
    data["altitude"] = sensors.altitude
...
def sensor_data_to_telemetry_format(parsed_sensor_data): # parsed sensor data is a dictionary
    ...
    data["HS_deployed"] = gl_HS_deployed
    data["PC_deployed"] = gl_PC_deployed
...
def deploy_heatshield():
    ...
    gl_state = "Descent"
    # Code to deploy the heatshield - electronics, mechanism (TODO)
    gl_HS_deployed = True
    #################### UNCOMMENT LATER ####################
...
def deploy_parachute():
    ...
    gl_state = "Landing"
    # Code to deploy the parachute - electronics, mechanism (TODO)
    gl_PC_deployed = True
    #################### UNCOMMENT LATER ####################
    parachute_servo(gl_parachute_servo_angle)
    # #######################################################

def release_heatshield():
    """
    Function to release the heatshield
    """
    global gl_HS_deployed, gl_state
    gl_state = "Descent"
    # Code to release the heatshield - electronics, mechanism (TODO)
    gl_HS_deployed = False
    #################### UNCOMMENT LATER ####################
    # HS_release_servo(gl_HS_deploy_servo_angle)
    # #######################################################
...
def turn_beacon_off():
    ...
    # Code to turn off the beacon - electronics (TODO)
    gl_beacon_status = "OFF"
    #################### UNCOMMENT LATER ####################
    # beacon_off()
    # #######################################################
...
def turn_beacon_on():
    ...
    """
    global gl_beacon_status
    # Code to turn on the beacon - electronics (TODO)
    gl_beacon_status = "ON"
    #################### UNCOMMENT LATER ####################
    # beacon_on()
    # #######################################################
...
def call_CANSAT_ops(command_name, arguments, parsed_sensor_data):
    ...
    """
    global gl_telemtry_status, gl_previous_command, gl_mode, gl_simp_pressure
...
if __name__ == "__main__":
    ...
    while True:
        ...
        parsed_data = get_message_from_P1()
        gl_state = parsed_data["state"]
        gl_mode = parsed_data["mode"]
